...
Redis keys are organized by canonical market-data subject rather than by UI screen. A quote is stored once per `boardId:symbol`, while market boards, categories, and watchlists resolve to symbol membership lists that point to those quote aggregates. Quote, symbol detail, index state, and board/session state use Redis Hashes with a canonical `payload` field plus materialized scalar fields for inspection and future field-level update optimization. Latest trades use Redis Lists. OHLC ranges use Redis Sorted Sets with coverage tokens so the API does not accidentally return a partial historical range as a full cache hit.

DNSE WebSocket events update their matching Redis subject before Pub/Sub fan-out. Trade, depth, security definition, foreign trading, and expected auction price enrich quote state. Actual and estimated market-index events enrich index state without overwriting each other. OHLC and closed-OHLC events upsert the relevant stock or index sorted set immediately, while range coverage remains a REST backfill responsibility. Session events update `session:{productGroupId:boardId}:state`.
...
- Symbol detail reads should synthesize their initial quote/depth/foreign snapshot from `quote:{boardId:symbol}:state` when a usable quote exists, instead of calling DNSE REST to fetch the same price fields again. Metadata-only backfill may call `/instruments` and `/price/{symbol}/secdef`, but it must not call trade, top-price, or foreign-trading endpoints just to open the detail panel. Metadata, OHLC, and latest trade history remain independent backfill concerns.
